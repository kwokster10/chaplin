...
app.get("/search", function(req, resp){
	var keywords = req.query.search_words.toUpperCase();
	...
	request("http://api.giphy.com/v1/gifs/search?q=" + keywords + "&limit="+limit_amt+"&api_key="+ api_key, function(err, response, body) {
		...
		resp.render("show.ejs", {words: keywords, gifs: gifs});
	});
});

app.get("/search/saved", function(req, resp) {
	var save_gif = req.query.save;
	resp.render("save.ejs", {save_gif : save_gif});
});
